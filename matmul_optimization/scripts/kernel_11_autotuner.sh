...
OUTPUT="../benchmark_results/kernel_11_autotune_results.csv"

# Clear the output file
echo "" > $OUTPUT
# add headers
echo "BK,BM,BN,WM,WN,WNITER,TM,TN,NUM_THREADS,SIZE,TIME,GFLOPS" >> $OUTPUT
...
TOTAL_CONFIGS="$(( ${#BK_VALUES[@]} * ${#BM_VALUES[@]} * ${#BN_VALUES[@]} * ${#WM_VALUES[@]} * ${#WN_VALUES[@]} * ${#WNITER_VALUES[@]} * ${#TM_VALUES[@]} * ${#TN_VALUES[@]} * ${#NUM_THREADS_VALUES[@]} * ${#MATRIX_SIZE[@]} ))"
CONFIG_NUM=0
            ...
            for TM in "${TM_VALUES[@]}"; do
              for TN in "${TN_VALUES[@]}"; do
                for NUM_THREADS in "${NUM_THREADS_VALUES[@]}"; do
                  ...
                  echo "($CONFIG_NUM/$TOTAL_CONFIGS): BK=$BK BM=$BM BN=$BN WM=$WM WN=$WN WN_ITER=$WN_ITER TM=$TM TN=$TN NUM_THREADS=$NUM_THREADS SIZE=$SIZE" #|& tee -a $OUTPUT
                  # output the current configuration parameters to the the CSV file
                  echo -n "$BK,$BM,$BN,$WM,$WN,$WN_ITER,$TM,$TN,$NUM_THREADS,$SIZE," >> $OUTPUT
                  # Run the benchmark and get the result
                  # Kill the program after 8 seconds if it doesn't finish
                  timeout -v 30 ./sgemm 11 $SIZE $OUTPUT
                  if [ $? -eq 124 ]; then
...
echo "Autotuning complete. Results written to $OUTPUT"