...
def plot(df: pd.DataFrame):
    ...
    sn.scatterplot(data=df, x="size", y="gflops",
                   hue="kernel", palette=colors, legend=False)
...
if __name__ == "__main__":
    ...
    df = df[df["size"] == 4096].sort_values(by="gflops", ascending=True)[
        ["kernel", "gflops"]]
    df["kernel"] = df["kernel"].map(
        {k: f"{k}: {v}" for k, v in KERNEL_NAMES.items()})
    df["relperf"] = df["gflops"] / \
        df[df["kernel"] == "0: cuBLAS"]["gflops"].iloc[0]
