...
int main(int argc, char **argv) {
    ...
    if (argc != 2) {
        std::cerr << "Select a kernel (range 0 -12, 0 for NVIDIA cuBLAS)"
                << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    // declare handle, create handle, cublasCreate will return
    // a value of type cublasStatus_t to determine whether the
    // handle was created successfully (value is 0)
    ...
    // cuBLAS FLOPs cieling is reached at 8192
    ...
    for (int size : SIZE) {
        ...
        std::cout << "dimesions(m=n=k) " << m << ", alpha: " << alpha << ", beta: " << beta << std::endl;
        // Verify correctness of the calculation and execute it once before
        ...
        if (kernel_num != 0) {
            ...
            if (!verify_matrix(C_ref, C, m * n)) {
                std::cout <<"Failed to pass correctness verification against NVIDIA cuBLAS" << std::endl;
                if (m <= 128) {
